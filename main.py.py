...
class Trapeze:
    def __init__(self, a, b, c, d):
        self.a, self.b, self.c, self.d = a, b, c, d  # a, b - основи | c, d - бічні сторони
        self.h = self._calculate_height()

    def _calculate_height(self):
        s = (self.c + self.d - self.a + self.b) / 2
        return 2 * math.sqrt(s * (s - self.c) * (s - self.d) * (s - (self.a - self.b))) / abs(self.a - self.b)
    ...
    for filename in ["input01.txt", "input02.txt", "input03.txt"]:
        try:
            with open(filename, "r") as f:
                for line in f:
                    shapes.append(parse_shape(line))
